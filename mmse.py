...
#        print(UA2.dot(scale).dot(UBT2))           
        eta[0] = 1/d(Q);
        gammam[0] = eta[0] - gammap[0]; 
        rm[0] = eta[0]*x[0]/(num_facnodes-1) - gammap[0]*rp[0];        
        
        #factor node 2=column sum check
        x[1] = rp[1] + (1/gammap[1])*(1-np.sum(rp[1],axis=0) )/np.sum(1/gammap[1],axis=0);
        Q = (1-(1/gammap[1])/np.sum(1/gammap[1],axis=0) )/gammap[1];       
        eta[1] = 1/d(Q);        
        gammam[1] = eta[1] - gammap[1];
        rm[1] = eta[1]*x[1]/(num_facnodes-1) - gammap[1]*rp[1];  

        #factor node 3 - row sum check
        x[2] = rp[2] + (1/gammap[2])*(1-np.sum(rp[2],axis=1) )/np.sum(1/gammap[2],axis=1);
        Q = (1-(1/gammap[2])/np.sum(1/gammap[2],axis=1) )/gammap[2];       
        eta[2] = 1/d(Q);        
        gammam[2] = eta[2] - gammap[2];
        rm[2] = eta[2]*x[2]/(num_facnodes-1) - gammap[2]*rp[2];  

        #factor node 4  - non-negativity        
        x[3] = rp[3] + (gammap[3]**(-0.5))*norm.pdf(rp[3]*(gammap[3]**0.5))/norm.sf(-rp[3]*(gammap[3]**0.5)) ;
        Q = (1-((norm.pdf(rp[3]*(gammap[3]**0.5))*(rp[3]*(gammap[3]**0.5) +  norm.pdf(rp[3]*(gammap[3]**0.5))) )/(norm.sf(-rp[3]*(gammap[3]**0.5))**2) )) /gammap[3]; 
        eta[3] = 1/d(Q);
        gammam[3] = eta[3] - gammap[3];
        rm[3] = eta[3]*x[3]/(num_facnodes-1) - gammap[3]*rp[3]; 
        print(gammap[3])
